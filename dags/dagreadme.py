...
path_to_local_home = "/opt/airflow/readme/" # we download in this location so file is is not deleted when the downloading task finishes
...
def fetch_readme(owner, repo, file_path='/opt/airflow/readme/', delay=1):
    """
    Fonction pour récupérer le contenu du fichier README d'un dépôt GitHub et l'écrire dans un fichier texte.
    ...
    if response.status_code == 200:
        ...
        file_path_with_name = file_path + f"README_{repo}.txt"
        
        with open(file_path_with_name, 'w') as file:
            file.write(content)
        print(f"Le contenu du README de {repo} a été écrit dans {file_path_with_name}.")
        ...
        if rate_limit_remaining == 0:
            ...
            fetch_readme(owner, repo, file_path, delay * 2)
    else:
        print(f"Erreur lors de la récupération du README: {response.status_code}")
...
with DAG(
    dag_id="readme_from_github_repos",
